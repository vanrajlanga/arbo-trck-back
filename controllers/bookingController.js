...
const createBooking = async (req, res) => {
  try {
    const { trekId, participants, pickupPointId, couponCode, specialRequests } =
      req.body;
...
const updateBooking = async (req, res) => {
  try {
    ...
    const updateData = {};
    if (pickupPointId !== undefined) updateData.pickup_point_id = pickupPointId;
...
const getBookingConfirmation = async (req, res) => {
  try {
    ...
    res.json({
      confirmation: {
        bookingId: booking.id,
        confirmationNumber: `ARB${booking.id.toString().padStart(6, "0")}`,
        trek: booking.trek,
    ...
    const analytics = await Promise.all([
      ...
      Booking.findAll({
        ...
        attributes: [
          ...
          [sequelize.fn("COUNT", sequelize.col("id")), "confirmedBookings"],
        ],
      }),
...
const verifyPaymentAndCreateBooking = async (req, res) => {
  try {
    ...
    if (couponCode) {
      ...
      if (coupon) {
        const discountAmount = (finalAmount * coupon.discount_percentage) / 100;
    ...
    const travelerPromises = travelers.map(async (travelerData, index) => {
      ...
      if (!traveler) {
        traveler = await Traveler.create({
          ...
          dietary_restrictions: travelerData.dietary_restrictions || null,
        });
      }
...
const createMobileTrekOrder = async (req, res) => {
  try {
    const { trekId, travelers, pickupPointId, couponCode, specialRequests } =
      req.body;
    ...
    for (let i = 0; i < travelers.length; i++) {
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          message: `Please fill all required fields for traveler ${i + 1}`,
        });
      }
    }
    ...
    const completeBooking = await Booking.findOne({
      ...
      include: [
        ...
        },
        {
          model: PaymentLog,
          as: "payments",
          attributes: ["amount", "payment_method", "transaction_id", "status"],
        },
      ],
    });
